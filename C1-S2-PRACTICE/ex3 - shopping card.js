...
const PRODUCTS = [
  ...
  function getCartTotalAmount() {
    let result = 0;
    SHOPPING_CART.map(element => {
      let product_id = element.id;
      let item = PRODUCTS.find(product => product.id === product_id);
      if (item) {
        result += item.price*element.quantity;
      }
    });
    return result;
  }
  ...
  function addProductToCart(productId) {
    let check = SHOPPING_CART.find((element) => element.id === productId);
    if (check) {
        check.quantity += 1;
    } else {
  ...
  function removeProductFromCart(productId) {
    let index = SHOPPING_CART.findIndex((element) => element.id === productId);
    if (index !== -1) { 
